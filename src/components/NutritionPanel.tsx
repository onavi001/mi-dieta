...
type CalculatorActivityLevel = 'sedentary' | 'light' | 'moderate' | 'active'
type CalculatorGoal = 'lose' | 'loseFast' | 'maintain' | 'muscle' | 'gain' | 'endurance' | 'healthy'

const OBJECTIVE_OPTIONS: Array<{ value: ObjectiveGoal; label: string; description: string }> = [
...
const ACTIVITY_LEVEL_OPTIONS: Array<{ value: ActivityLevel; label: string; factor: number; description: string }> = [
  ...
  { value: 'moderately_active', label: 'Moderadamente activo', factor: 1.55, description: 'Ejercicio moderado 3-5 días a la semana' },
  { value: 'very_active', label: 'Muy activo', factor: 1.725, description: 'Ejercicio intenso 6-7 días a la semana' },
]

const GOAL_CALORIE_ADJUSTMENTS: Record<ObjectiveGoal, number> = {
  weight_loss: -500,
...
const GOAL_MACRO_RATIOS: Record<ObjectiveGoal, { protein: number; carbs: number; fat: number }> = {
...
function FieldShell({ id, label, helper, children, className = '' }: FieldShellProps) {
  return (
...
function CollapsibleCard({ title, stepNumber, isExpanded, onToggle, isDisabled = false, isComplete = false, children }: CollapsibleCardProps) {
  return (
    <div className={`border rounded-2xl overflow-hidden transition-all ${isDisabled ? 'bg-gray-100 border-gray-200' : 'bg-white border-gray-200'}`}>
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${isComplete ? 'bg-emerald-100 text-emerald-700' : isExpanded ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
...
function StepIndicator({ currentStep, totalSteps, stepLabel }: StepIndicatorProps) {
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-3">
      <div className="flex items-center justify-between gap-2 mb-2">
        <p className="text-xs font-semibold text-gray-600 uppercase">Progreso</p>
        <span className="text-xs font-bold text-emerald-600">{currentStep}/{totalSteps}</span>
      </div>
      <div className="w-full bg-gray-200 rounded-full h-2">
        <div className="bg-emerald-600 h-2 rounded-full transition-all" style={{ width: `${(currentStep / totalSteps) * 100}%` }} />
      </div>
...
function TextareaField({ id, label, value, onChange, placeholder, helper }: TextareaFieldProps) {
  return (
...
function SelectField({ id, label, value, onChange, helper, children }: SelectFieldProps) {
  return (
    <FieldShell id={id} label={label} helper={helper}>
      <select
        id={id}
