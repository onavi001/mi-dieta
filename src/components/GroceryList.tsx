...
import { generateGroceryListFromMeals } from '../data/groceryEngineV2'
...
export function GroceryList({ accessToken, meals, groceryState, weekState, onChangeGroceryState, onSyncWeekState, isSavingState = false }: GroceryListProps) {
  ...
  }, [meals, summary?.nutritionProfile?.allergies, summary?.nutritionProfile?.food_preferences, summary?.nutritionProfile?.intolerances])
  ...
  }, [filteredMeals, meals, summary?.nutritionProfile?.allergies, summary?.nutritionProfile?.food_preferences, summary?.nutritionProfile?.intolerances])

  const SUPER = useMemo(() => generateGroceryListFromMeals(filteredMeals), [filteredMeals])
