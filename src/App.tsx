...
export default function App() {
  ...
  const autoGeneratedWeekKeyRef = useRef<string>('')
      ...
      const rankedOverrideEntries = await Promise.all(
        generatedPlan.slots
          ...
          .map(async (slot) => {
            ...
            const alternatives = await fetchSlotAlternatives(slot.slot, slot.meal.id)
  ...
  useEffect(() => {
    if (tab !== 'dieta' || viewMode !== 'my' || loading) return

    const userId = session?.user.id
    if (!userId) return

    const hasMeals = activeSlots.some((slot) => Boolean(slot.meal))
    if (hasMeals) return

    const currentWeek = plan?.week || 'no-week'
    const generationKey = `${userId}:${currentWeek}`
    if (autoGeneratedWeekKeyRef.current === generationKey) return
    ...
    void (async () => {
      try {
        ...
        if (generated) {
          autoGeneratedWeekKeyRef.current = generationKey
        }
      } finally {
        setAutoGeneratingMeals(false)
      }
    })()
  }, [activeSlots, generatePlanWithRetry, loading, plan?.week, session?.user.id, tab, viewMode])
                  ...
                  {ownedShareUsers.map((item) => (
                    <div key={item.profile.id} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-2 py-1.5">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        >
                          {item.relation?.canEdit ? 'Solo ver' : 'Puede editar'}
                        </button>
                        <button
                          type="button"
              ...
              ? (
                <div className="px-4 py-8 space-y-4">
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl p-5">
                    ...
                    <button
                      type="button"
                      onClick={() => void generatePlanWithRetry()}
                      disabled={loading || autoGeneratingMeals}
                      className="mt-3 w-full bg-gray-900 text-white rounded-xl py-2.5 text-sm font-semibold disabled:opacity-60"
                    >
                      {loading || autoGeneratingMeals ? 'Generando...' : 'Generar mi dieta personalizada'}
                    </button>
                  </div>
                </div>
              )
