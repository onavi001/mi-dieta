...
import { DATA_RESET_ALLOWED_USER_ID } from '@/constants/app'
...
import { LegalSheet, type LegalView } from '@/features/legal/LegalSheet'

type DietMode = 'today' | 'week'

const WeeklyDiet = lazy(async () => {
  const mod = await import('@/features/diet-planner/WeeklyDiet')
  return { default: mod.WeeklyDiet }
})

const GroceryList = lazy(async () => {
  const mod = await import('@/features/grocery/GroceryList')
  return { default: mod.GroceryList }
})

const NutritionPanel = lazy(async () => {
  const mod = await import('@/features/nutrition/NutritionPanel')
...
export default function App() {
  ...
  const handleGeneratePlan = useCallback(async (): Promise<boolean> => {
    try {
      ...
      try {
        const nutritionData = (summary || (await loadSummary())) ?? null
        const catalogMeals = await fetchAllMealsCatalog()
        const mealsByTipo = groupMealsByTipo(catalogMeals)
        const { applyPostPlanGenerationOptimizations } = await import(
          '@/services/plan-generator/applyPostPlanGenerationOptimizations'
        )
        return await applyPostPlanGenerationOptimizations({
          generatedPlan,
          hasGeneratedMeals,
          nutritionData,
          weekStateSuggestionPreferences: weekState?.suggestionPreferences,
          mealsByTipo,
          replaceIngredient,
          syncWeekState,
        })
    ...
    return (
      <>
        <div className="min-h-screen bg-linear-to-b from-lime-50 via-amber-50 to-white flex items-center justify-center p-4">
          ...
          <h1 className="text-xl font-semibold text-gray-900 mb-1">Mi Dieta</h1>
